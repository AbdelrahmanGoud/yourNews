...
{  
          ...
          useEffect(() => {
            const fetchData = async () => {
              try {
                const response = await fetch(`https://newsapi.org/v2/top-headlines?country=us&category=${categoryn}&apiKey=bb527eb5ac6a4b42a17654fe3f8d4d59`);
   ...
   return(
     <>
      <section className="sec-1 ">
        ...
        <div className="content text-center text-black px-5">
          ...
          <p></p>
          <a href="#us-sec-2" className="lin-sec-1">
            <i className="fa-solid fa-angles-down icon-sec-1"></i>
          </a>
        </div>
      </section>
      <section className='us-sec-2 py-5' id='us-sec-2'>
      <div className=''>
    ...
    {usnews.map((us)=>(
     ...
     <Card.Body>
       <Card.Title className='mb-3'>{us.author}</Card.Title>
       <Card.Text>
         {us.description}
       </Card.Text>
       <Card.Link href={us.url}>Details</Card.Link>
